...
* pynumpy is about 100 times slower than numpy
...
def measure_mem(what, code, divide=1):
    ...
    print("Memory for %s:%s%s" % (what, " " * (22 - len(what)), _prettymem(m)))
...
if __name__ == "__main__":
    ...
    measure_mem(
        "ints 0-255", "L = [int(random.uniform(0, 255)) for i in range(N)]", N
    )
    ...
    measure_mem(
        "    Numpy arr size 1", "L = [np.ones((1,)) for i in range(N)]", N
    )
    measure_mem(
        "Pynumpy arr size 1", "L = [pnp.ones((1,)) for i in range(N)]", N
    )

    measure_mem("    Numpy arr size M", "a = np.ones((M,))")
    measure_mem("Pynumpy arr size M", "a = pnp.ones((M,))")
    ...
    measure_speed("pynumpy sum 10k", a2.sum)
    measure_speed("    numpy max 10k", a1.max)
    measure_speed("pynumpy max 10k", a2.max)

    a1 = np.ones((1000, 1000))
    a2 = pnp.ones((1000, 1000))
    measure_speed("    numpy sum 1M", a1.sum)
    measure_speed("pynumpy sum 1M", a2.sum)
    measure_speed("    numpy max 1M", a1.max)
    measure_speed("pynumpy max 1M", a2.max)
